...
const PersonSchema = new mongoose.Schema(
  {
    ...
    preacher: { type: String, enum: [...PREACHER_VALUES, ''], default: '' },
  },
  { _id: false }
);
...
const RegistrationSchema = new mongoose.Schema(
  {
    ...
    // there is only one status. Kept as a field (rather than removed
    // outright) so the gate and admin code that already checks `status`
    // needs no structural change, and so older paid-era documents (if any
    // exist from testing) don't break existing queries.
